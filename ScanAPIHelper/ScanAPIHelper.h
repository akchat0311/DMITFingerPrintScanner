...
namespace ScanAPIHelper {
    ...
    public value struct DeviceInfo
    {
        ...
        static initonly DeviceInfo Empty = { 0,Size(0, 0)};
    };
	...
	public:
		///<summary>
		///ftrMathAPI function
		///  FTR_BOOL ftrMathImageNfIQ ( FTRHANDLE ftrHandle, FTR_PVOID pBuffer, int nWidth, int nHeight, int *nNfiq, FTR_BOOL *bErrNfiq );
		///</summary>
		int Math_ImageNfiq( array<Byte>^ pImage, int nWidth, int nHeight )
		{
            CheckDispose();
            if( m_hDevice == NULL )
            {
			...
			int nNfiq = 0;
			FTR_BOOL bErrNfiq = FALSE;
			pin_ptr< Byte > pBuffer = &pImage[0];
			if( !ftrMathImageNfIQ( m_hDevice, pBuffer, nWidth, nHeight, &nNfiq, &bErrNfiq ) )
			{
				m_LastErrorCode = (int)GetLastError();
                throw gcnew ScanAPIException( m_LastErrorCode );
			}
